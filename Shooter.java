...
public class Shooter extends Agent implements Runnable{
	public static final double MOVE = 5.0;
	public static final double SHOOT_DIST = 200.0;
	Pos[] myTeam, otherTeam;
	int nMyTeam, nOtherTeam;
	int myPos;
	Pos target;
	Pilota ball;
	boolean haveBall, ballShot;
	
	//Constructor de classe.
	public Shooter(int lN, int lS, int lE, int lO, int nteamA, int nteamB, int team, int pos, Pilota ball) {
	...
	//Funcio que considera si esta en posicio de xut.
	private boolean shootPosition() {
	...
	//Funcio que xuta la pilota.
	private void shoot() {
		haveBall = false;
		double distTarg = Math.sqrt( Math.pow( target.x - my_x,2 ) + Math.pow( target.y - my_y,2 ) );
		pilota.x = (int)(my_x + (target.x - my_x)/distTarg*15.0);
		pilota.y = (int)(my_y + (target.y - my_y)/distTarg*15.0);
		ball.setPilota( pilota.x, pilota.y );
		ball.setTargX( target.x );
		ball.setTargY( target.y );
	} //Fi shoot.
